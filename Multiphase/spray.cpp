...
//	mscene = SCENE_MELTING;
	mscene = SCENE_LBMWATER;

	stiffness = 0.05;  //   gy stiffness!
	SolidbounceParam = 0.05f;	//固体碰到边界时反弹的参数 //这个值起到的作用和刚性系数类似
	mpause = false;
	boutputpovray = false;					//一开始就改 否则找不到路径
	boutputobj = false;
	...
	else if (mscene == SCENE_MELTING)
	{
		...
		simmode = SIMULATION_BUBBLE;
		solidInitPos = make_float3(0.8f, 0.5f, -0.25f);				//固体位置
	...
	else if (mscene == SCENE_MELTINGPOUR)
	{
		...
		pourvel = make_float3(-.8f, 0.0f, 0.f);
		...
		simmode = SIMULATION_BUBBLE;
		solidInitPos = make_float3(0.55f, 0.45f, -0.2f);				//固体位置
	...
	else if (mscene == SCENE_LBMWATER)//////////////////LBM water setting
	{
		m_bMelt = false;
		m_bFreeze = false;
		m_bFixSolid = false;
		...
		if (boutputobj)
			sprintf(outputdir, "outputobj\\objinteraction\\");
		simmode = SIMULATION_LBMWATER;
		//simmode = SIMULATION_LBMWATER;
		solidInitPos = make_float3(0.8f, 0.5f, -0.f);
	...
	hparam.dt = 0.002f;		//timestep 原为67  注意：pouring与时间步长有关
...
{	
	...
	initparam();	//Init FLIP and LBM
	if(simmode != SIMULATION_LBMWATER)
	initmem_bubble();
	else LBMinitmemset();//为LBM 开辟内存
	printf("initmem complete.\n");
	
	if(simmode == SIMULATION_BUBBLE)
	initEmptyBubbles();		//决定emptybubble的数量，并分配存储

	initParticleGLBuffers();//GLSL setting
	...
	//	initscene_bubble();
	//else
	initparticle_solidCoupling(); //init fluid partices both for FLIP &LBM
...
void cspray::simstep()
{
	if (simmode == SIMULATION_WATER) 
		...
		heatsim();
	else if (simmode == SIMULATION_LBMWATER)
		LBMwatersim();
...
void cspray:: LBMwatersim()//LBM prpcess  Note：common FLIP， not multiFLIP
{
	if (!mpause)
	{
		CTimer time, time2;
