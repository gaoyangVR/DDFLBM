...
void cspray::initparticle_solidCoupling()///////////////////initparticle 初始化流体粒子
{
	...
	float scale = 100;
	if (mscene == SCENE_FREEZING || mscene == SCENE_MELTINGPOUR) scale = 50;
...
{
	int fluidParCntPerGridThres = 10;
	markfluid_LBMdense << <pblocknum, threadnum >> >(mmark, parmass, parflag, parNumNow, gridstart, gridend, fluidParCntPerGridThres);
	cudaThreadSynchronize();
	getLastCudaError("Kernel execution failed");
...
{
	initphi << <gsblocknum, threadnum >> >(phi, mmark, typeflag);//grid phi -0.5 其他3*NX
	cudaThreadSynchronize();
	getLastCudaError("Kernel execution failed");
...
void cspray::sweepU(farray ux, farray uy, farray uz, farray phi, charray mark, char typeflag)
{
	...
	{
		if(simmode==SIMULATION_LBMWATER)
...
void cspray::runMC_interaction()
{
	if (mframe>0 && !bRunMCSolid)//just output, they are updated in solidmotion function.
	{
		...
		if (boutputobj && mframe%outputframeDelta == 0)
			outputBOBJwater(mframe / outputframeDelta, solidvertex, solidnormal, solidvertexnum, solidindices, solidindicesnum, "solid");
	}
	else
		runMC_smooth("solid", TYPESOLID);
}
...
void cspray::runMC_smooth(const char* objectname, char MCParType)
{
	...
	if (activeVoxels == 0) {
		...
		if (boutputobj && mframe%outputframeDelta == 0)
			outputBOBJwater(mframe / outputframeDelta, NULL, NULL, 0, NULL, 0, objectname);
		return;
	}
...
void cspray::outputColoredParticle(int frame, float3* dpos, float *dtemperature, int pnum)
{
	...
	float iradius = 0.004f;//临时改的
	float3 color;
	for (int i = 0; i<pnum; i+= 1)
	{
		if (hparflag[i] == TYPESOLID) color = make_float3(0., 0., .80);
		else color = make_float3(.8, .8, 0.);
...
//////////////////////////////
	...
	float wsum=0.;
	int snum = 0; //统计固体粒子
	float3 evedis =make_float3(0.);//平均距离
 	for (int i = 0; i < parNumNow; i++)
	...
	evedis /= snum;		//geometry of shape
//	printf("%f %f %f", evedis.x, evedis.y, evedis.z); getchar();
...
//	restCM = accumulate_GPU_f3(solidParPos,solidParMass);
	...
	if (hparflag[i] == TYPESOLID)
	{
		shapesum += x0[i] - evedis;
	}
	shapesum /= snum;

 	for (int i = 0; i < parNumNow; i++)// 删除边界粒子
 	if (hparflag[i] == TYPESOLID)
 	{
 		if (abs(x0[i].x - restCM.x) > 2.0*evedis.x || abs(x0[i].y - restCM.y) > 2.0*evedis.y || abs(x0[i].z - restCM.z) > 2.0*evedis.z)
 		{
 			hparflag[i] = TYPEFLUID;
 			x0[i] = restCM + make_float3(rand() / 100.0, rand() / 100.0, rand() / 100.0 );
 			newparvel[i] = make_float3(0.);
 		}
 	}
	//A
	matrix3x3 A;
...
void cspray::LBMinitmemset()
{
	...
	int gvymemsize = sizeof(float)*hparam.gvnum.y;
	int gvzmemsize = sizeof(float)*hparam.gvnum.z;
	cudaMalloc((void**)&waterux.data, gvxmemsize);	//waterux
	waterux.setdim(NX + 1, NY, NZ);
	cudaMalloc((void**)&wateruy.data, gvymemsize);
	wateruy.setdim(NX, NY + 1, NZ);
	cudaMalloc((void**)&wateruz.data, gvzmemsize);
